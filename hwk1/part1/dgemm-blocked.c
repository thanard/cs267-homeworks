...
static void avx_mult(double* A, double* B, double* C, int lda){
  ...
  // C+4
  tmp = _mm256_mul_pd(a1, _mm256_broadcast_sd(B+lda));
  tmp = _mm256_fmadd_pd(a2, _mm256_broadcast_sd(B+lda+1), tmp);
  tmp = _mm256_fmadd_pd(a3, _mm256_broadcast_sd(B+lda+2), tmp);
  tmp = _mm256_fmadd_pd(a4, _mm256_broadcast_sd(B+lda+3), tmp);
  _mm256_store_pd(C+4, _mm256_add_pd(_mm256_loadu_pd(C+4), tmp));    

  // C+8
  tmp = _mm256_mul_pd(a1, _mm256_broadcast_sd(B+2*lda));
  tmp = _mm256_fmadd_pd(a2, _mm256_broadcast_sd(B+2*lda+1), tmp);
  tmp = _mm256_fmadd_pd(a3, _mm256_broadcast_sd(B+2*lda+2), tmp);
  tmp = _mm256_fmadd_pd(a4, _mm256_broadcast_sd(B+2*lda+3), tmp);
  _mm256_store_pd(C+8, _mm256_add_pd(_mm256_loadu_pd(C+8), tmp));


  // C+4
  tmp = _mm256_mul_pd(a1, _mm256_broadcast_sd(B+3*lda));
  tmp = _mm256_fmadd_pd(a2, _mm256_broadcast_sd(B+3*lda+1), tmp);
  tmp = _mm256_fmadd_pd(a3, _mm256_broadcast_sd(B+3*lda+2), tmp);
  tmp = _mm256_fmadd_pd(a4, _mm256_broadcast_sd(B+3*lda+3), tmp);
  ...
  for (int j = 0; j < N; j += 4)
    ...
    for (int i = 0; i < M; i+=4)
    {
      ...
      for (int k = 0; k < K; k += 4)
      {
        ...
        avx_mult(A+i+k*lda, B+k+j*lda, tempC, lda);
      }
